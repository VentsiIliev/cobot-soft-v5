...
from modules.shared.tools.glue_monitor_system.config import (
    UPDATE_SCALE_ENDPOINT, TARE_ENDPOINT, GET_CONFIG_ENDPOINT, UPDATE_OFFSET_ENDPOINT
)
...
class LoadCellsSettingsTabLayout(BaseSettingsTabLayout, QVBoxLayout):
    ...
    def _parse_cell_from_dto(self, cell_dto):
        ...
        print(f"Parsed cell {cell_id}: {cell_dto['type']}, capacity={cell_dto['capacity']}g")
    ...
    def fetch_calibration_config(self, load_cell_id: int):
        ...
        # First, try to get from already loaded cell configs (from endpoints)
        if load_cell_id in self.cell_configs:
            ...
            print(f"Using cached calibration for cell {load_cell_id}: offset={offset}, scale={scale}")
            return {"offset": offset, "scale": scale}

        # Fallback: Try to fetch from hardware if glue_cells_manager is available
        if self.glue_cells_manager and hasattr(self.glue_cells_manager, 'config_data'):
            ...
            print(f"Fetching calibration config from hardware: {url}")
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Calibration fetched from hardware: offset={offset}, scale={scale}")
                    return {"offset": offset, "scale": scale}
                else:
                    print(f"Failed to fetch calibration config: {response.status_code}")
            except Exception as e:
                print(f"Error fetching calibration config from hardware: {e}")
